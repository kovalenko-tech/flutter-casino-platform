...
C4Container
    ...
    System_Boundary(app, "Flutter Casino App") {
        ...
        ContainerDb(sqlite_db, "SQLite Database", "SQLite (embedded SQL)", "On-device persistent store. Holds user records, game catalogue, banners, and history.")
    }
    ...
    Rel(data, sqlite_db, "CRUD operations", "sqflite")
    ...
    UpdateElementStyle(sqlite_db, $bgColor="#999999", $fontColor="#ffffff")
```
...
| **SQLite DB** | On-device `.db` file | Managed entirely by SQLite runtime |
